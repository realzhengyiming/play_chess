  (async()=>{
    ...
    const DEFAULT_COUNTS={bedroom:{bed:1,night:2,wardrobe:1,desk:1,chair:1,bench:0,chest:0,shelf:0,tvbench:0,bedroomDisplay:0,lounge:0,bedroomLoveseat:0,bedroomTeaTable:0,vanity:0,vanityStool:0,bedroomInfillCabinet:1},living:{sofa:1,tv:1,coffee:1,diningTable:0,diningChair:0,sideboard:0,bookcase:0,display:0,console:0,arm:2,ottoman:0,side:1,floorLamp:1,plant:0,infillCabinet:1,livingRug:0}};
    const DEFAULT_DESIGN_QUALITY_RULES={
      version:2,weights:{function:.25,ground:.25,wall:.20,relation:.15,circulation:.15},
    ...
    const DEFAULT_LAYOUT_CONSTRAINTS={
      ...
      inventory:{
        roomAreaModules:{
          bedroom:[{id:'micro-bedroom',label:'微型卧室',minArea:0,modules:['sleep']},{id:'sleep',label:'睡眠卧室',minArea:9,modules:['sleep']},{id:'work-bedroom',label:'工作卧室',minArea:12,modules:['sleep','work']},{id:'suite-lounge',label:'套房会客',minArea:20,modules:['sleep','work','lounge']},{id:'studio',label:'超大单间',minArea:32,modules:['sleep','work','lounge','storage']}],
          living:[{id:'compact-living',label:'紧凑会客厅',minArea:0,modules:['conversation']},{id:'living',label:'标准会客厅',minArea:14,modules:['conversation','guest-seating']},{id:'living-dining',label:'客餐厅',minArea:24,modules:['conversation','guest-seating','dining']},{id:'grand-living-dining',label:'大客餐厅',minArea:34,modules:['conversation','guest-seating','dining','storage']}]
        },
        richMinimum:{
          bedroom:[{minArea:0,value:2,irregular:2},{minArea:8,value:3,irregular:3},{minArea:10,value:4,irregular:4},{minArea:15,value:6,irregular:6},{minArea:17,value:7,irregular:7},{minArea:20,value:7,irregular:7}],
          living:[{minArea:0,value:3,irregular:3},{minArea:16,value:6,irregular:6},{minArea:24,value:8,irregular:7},{minArea:34,value:10,irregular:9}]
        }
      },
      qualityPass:{
        minimumScores:{circulation:55,relation:62,composition:50,comfort:50,preference:45},
        requiredModuleScore:{bedroom:[{minArea:0,value:65}],living:[{minArea:0,value:65},{minArea:34,value:97}]},
      ...
      designGrammar:{
        living:{groups:{lounge:{anchor:'sofa',members:['sofa','tv','coffee','arm','side','ottoman','floorLamp'],compact:[.12,.30]},dining:{anchor:'diningTable',members:['diningTable','diningChair','sideboard'],compact:[.10,.25]}},zones:{wide:{lounge:[.34,.49],dining:[.73,.51]},deep:{lounge:[.50,.34],dining:[.51,.73]}},pairs:{sofaCoffee:{gap:[.40,.45],compact:[.30,.50],hardMin:.28},sofaTv:{gap:[1.55,3.20]},conversation:{radius:[.82,1.62]},diningSideboard:{distance:[.65,3.0]}}},
        bedroom:{groups:{sleep:{anchor:'bed',members:['bed','night','bench'],compact:[.10,.27]},work:{anchor:'desk',members:['desk','chair'],compact:[.06,.18]},lounge:{anchor:'bedroomLoveseat',members:['bedroomLoveseat','bedroomTeaTable','lounge'],compact:[.08,.22]}},zones:{wide:{sleep:[.36,.48],work:[.75,.42]},deep:{sleep:[.48,.38],work:[.72,.72]}},pairs:{nightBed:{gap:[0,.08]},benchBed:{gap:[.24,.58]}}}
      },
      search:{
      ...
      postLayout:{wallComplements:{enabled:true,countTowardRichMinimum:true,maxRecoverableIslandArea:.20,allowProgressiveIslandRepair:true,minIslandImprovement:.015,minUnusedWallImprovement:.04,maxWallScoreDrop:.02,closureExtensionMax:.12,dedupeClearance:.08,candidateMultiplier:3,priorityUsefulWidth:1.2,closureReserve:2,programs:{
    ...
    const row=(program,id,label,width,depth,category,role,color,rule={})=>({
      ...
      service:{label:'日常使用区',side:'front',depth:.42,spanExtra:0,hard:false,allowBodyTypes:[]},
    ...
    const defaults=[
      ...
      row('bedroom','chair','工作椅',.5,.5,'床组与座椅','工作组','#59a391',{patch:{candidate:{rules:[candidateRule('chair-desk-front','relation',{relativeTo:'desk',relation:'desk-front',side:'front',distance:{min:.03,max:.07,step:.02},facing:['toward'],maxSamples:6,weight:1.25,collisionClearance:.015})],maxCandidates:6},service:{label:'工作椅占位',side:'back',depth:0,spanExtra:0,hard:false,allowBodyTypes:[]}}}),
      ...
      row('bedroom','vanityStool','梳妆凳',.42,.42,'床组与座椅','梳妆组','#b78aa0',{patch:{candidate:{rules:[candidateRule('stool-vanity-front','relation',{relativeTo:'vanity',relation:'vanity-seat',side:'front',distance:{min:.22,max:.36,step:.07},facing:['toward'],maxSamples:8})],maxCandidates:8},service:{label:'坐凳后退区',side:'back',depth:.6,spanExtra:0,hard:true,allowBodyTypes:[]}}}),
      ...
      row('bedroom','bench','床尾凳',1.1,.4,'床组与座椅','床尾辅助','#6686a2',{patch:{candidate:{rules:[candidateRule('bench-bed-foot-near','relation',{relativeTo:'bed',relation:'bed-foot',side:'front',distance:{min:.10,max:.15,step:.05},facing:['toward'],maxSamples:4,allowFunctionalOverlap:true,weight:1.42})],maxCandidates:4},service:{label:'床尾凳共享落脚区',side:'front',depth:.42,spanExtra:0,hard:false,allowBodyTypes:[],sharedCirculation:true,adaptiveFootZone:true,adaptiveGapThreshold:.30}}}),
      ...
      row('bedroom','tvbench','卧室电视柜',1.2,.4,'收纳家具','视听收纳','#505f69',{requiredAnchor:'wall',avoidWindow:true,run:{min:1,max:2.6,step:.1,fill:[.58,.9]},patch:{accessTarget:true,candidate:{rules:[candidateRule('tvbench-bed-facing','relation',{relativeTo:'bed',relation:'bedroom-tv-bed-facing',side:'front',distance:{min:.45,max:2.8,step:.15},facing:['toward'],maxSamples:24,weight:2.2,referenceShapePolicy:{lShape:{enabled:true,lateralSide:'any',frontAlign:'body-center'}}}),candidateRule('tvbench-loveseat-facing','relation',{relativeTo:'bedroomLoveseat',relation:'bedroom-media-facing',side:'front',distance:{min:.55,max:2.8,step:.25},facing:['toward'],maxSamples:18,weight:1.55,referenceShapePolicy:{lShape:{enabled:true,lateralSide:'any',frontAlign:'body-center'}}}),candidateRule('tvbench-wall','wall',{maxSamples:40})],maxCandidates:56},service:{label:'设备取用区',side:'front',depth:.4,spanExtra:0,hard:false,allowBodyTypes:[]}}}),
      ...
      row('bedroom','bedroomTeaTable','卧室圆几',.56,.56,'休闲会客','茶歇中心','#b78b55',{patch:{candidate:{rules:[candidateRule('tea-loveseat-front','relation',{relativeTo:'bedroomLoveseat',relation:'lounge-table',side:'front',distance:{min:.3,max:.55,step:.125},facing:['parallel'],maxSamples:8,weight:1.25}),candidateRule('tea-chair-front','relation',{relativeTo:'lounge',relation:'lounge-table',side:'front',distance:{min:.24,max:.46,step:.11},facing:['parallel'],maxSamples:8})],maxCandidates:16},service:{label:'圆几取用区',side:'front',depth:.24,spanExtra:0,hard:false,allowBodyTypes:[]}}}),
      ...
      row('living','coffee','茶几',1.2,.6,'会客核心','会客中心','#bd9252',{patch:{quantity:{min:1,max:1},candidate:{rules:[candidateRule('coffee-sofa-front','relation',{relativeTo:'sofa',relation:'sofa-front',side:'front',distance:{min:.4,max:.8,step:.2},facing:['toward','away'],maxSamples:10,weight:1.5})],maxCandidates:12},service:{label:'茶几取用区',side:'front',depth:.6,spanExtra:0,hard:false,allowBodyTypes:[]}}}),
      row('living','arm','单人沙发',.8,.8,'座椅与小件','围合座位','#d7895d',{patch:{quantity:{min:0,max:4},candidate:{rules:[candidateRule('arm-sofa-opposite','relation',{relativeTo:'sofa',relation:'conversation-opposite',side:'front',distance:{min:.7,max:1.3,step:.2},facing:['toward'],maxSamples:8,weight:1.2,referenceShapePolicy:{lShape:{enabled:false,lateralSide:'any',frontAlign:'body-center'}}}),candidateRule('arm-side-front','relation',{relativeTo:'side',relation:'sofa-side-chair-chain',side:'front',distance:{min:.04,max:.16,step:.06},facing:['parallel'],maxSamples:10,weight:1.9,allowFunctionalOverlap:true,compoundConstraint:{ancestorRelativeTo:'sofa',side:'front',gap:.04}}),candidateRule('arm-open-zone','zone',{relation:'conversation-open-zone',maxSamples:16,weight:1.05}),candidateRule('arm-wall','wall',{relation:'conversation-wall',maxSamples:16,weight:.92})],maxCandidates:36},service:{label:'单椅起坐区',side:'back',depth:.6,spanExtra:0,hard:false,allowBodyTypes:[]}}}),
      ...
      row('living','diningTable','餐桌',1.4,.8,'客餐家具','客餐厅用餐','#8f704d',{patch:{accessTarget:true,candidate:{mode:'zone',rotations:[0,90],relativeTo:'',relation:'dining-zone',gap:0},service:{label:'餐桌使用区',side:'front',depth:.6,spanExtra:0,hard:false,allowBodyTypes:[]}}}),
      row('living','diningChair','餐椅',.46,.5,'客餐家具','餐桌组','#b08a64',{patch:{quantity:{min:0,max:6},candidate:{rules:['front','back','left','right'].map(side=>candidateRule(`dining-${side}`,'relation',{relativeTo:'diningTable',relation:'dining-seat',side,distance:{min:.08,max:.14,step:.06},facing:['toward'],maxSamples:4})) ,maxCandidates:20},service:{label:'餐椅后退区',side:'back',depth:.42,spanExtra:0,hard:false,allowBodyTypes:[]}}}),
      row('living','sideboard','餐边柜',1.6,.45,'沿墙柜体','餐储与台面','#8b6a4e',{requiredAnchor:'wall',run:{min:1,max:3.2,step:.1,fill:[.66,.96]},patch:{quantity:{min:0,max:2},accessTarget:true,service:{label:'柜前取用区',side:'front',depth:.5,spanExtra:0,hard:false,allowBodyTypes:[]}}}),
    ...
    function normalizeProfile(row){
      const rawCatalog=normalizeCatalog(row.catalog).filter(rule=>rule.id!=='bedroomRug'),upgradeIds=['bedroomLoveseat','bedroomTeaTable','bedroomDisplay'];
      const upgrading=row.richBedroomDefaultsV1!==true;
      ...
      if(row.bedroomCorePriorityV1!==true){
        const canonical=['bed','night','wardrobe','desk','chair','bench','tvbench','bedroomLoveseat','bedroomTeaTable','chest','shelf','bedroomDisplay','lounge','vanity','vanityStool','bedroomInfillCabinet'];
      ...
      if(row.livingCorePriorityV1!==true){
        const canonical=['sofa','tv','coffee','arm','side','floorLamp','diningTable','diningChair','sideboard','bookcase','display','console','ottoman','plant','infillCabinet','livingRug'];
      ...
      if(row.livingGroupDependencyV2!==true){const current=roomAssignments.living||[],rank=new Map(['sofa','tv','coffee','arm','side','floorLamp','diningTable','diningChair','sideboard','bookcase','display','console','ottoman','plant','infillCabinet','livingRug'].map((id,index)=>[id,index]));roomAssignments.living=[...current].sort((a,b)=>(rank.get(a)??999)-(rank.get(b)??999)||current.indexOf(a)-current.indexOf(b));roomAssignments.living.forEach((id,index)=>{roomSettings.living||={};roomSettings.living[id]={...(roomSettings.living[id]||{}),priority:(index+1)*10}})}
      if(row.bedroomGroupDependencyV3!==true){const current=roomAssignments.bedroom||[],rank=new Map(['bed','night','wardrobe','desk','chair','bench','tvbench','bedroomLoveseat','bedroomTeaTable','chest','shelf','bedroomDisplay','lounge','vanity','vanityStool','bedroomInfillCabinet'].map((id,index)=>[id,index]));roomAssignments.bedroom=[...current].sort((a,b)=>(rank.get(a)??999)-(rank.get(b)??999)||current.indexOf(a)-current.indexOf(b));roomAssignments.bedroom.forEach((id,index)=>{roomSettings.bedroom||={};roomSettings.bedroom[id]={...(roomSettings.bedroom[id]||{}),priority:(index+1)*10}})}
      ...
      for(const [key,rule] of Object.entries(defaultsByRule))defaultsByRule[key]=normalizeRule(rule);for(const rule of catalog)defaultsByRule[ruleKey(rule)]||=clone(rule);return {...row,baselineVersion:BASELINE_VERSION,richBedroomDefaultsV1:true,richBedroomDefaultsV2:true,wallCabinetFirstV1:true,bedroomCorePriorityV1:true,livingCorePriorityV1:true,livingGroupDependencyV2:true,bedroomGroupDependencyV3:true,catalog,defaultsByRule,roomTypes,roomAssignments,roomSettings};
    }
    ...
    let catalog=clone(profiles.find(profile=>profile.id===activeProfileId).catalog),selectedId=catalog[0]?.id||null,original=null,autoSaveTimer=null,remoteSaveTimer=null,serverReady=false,candidateRulesDraft=[],selectedCandidateRuleIndex=0,activeRoomTypeId='bedroom',previewVariantId='base',previewMode='item',viewMode='library';
    ...
    function writeLShapePolicy(entry,policy=lShapePolicy(entry)){
      ...
      // 旧字段继续写出一段过渡期，旧页面或历史脚本仍能读取同一语义。
      entry.excludeForLShape=!policy.enabled;entry.avoidChaiseSide=policy.lateralSide==='non-chaise';entry.lShapeCrossAlign=policy.frontAlign==='main-seat'?'main-seat':'bbox';
      return entry;
    }
    ...
    function normalizeCandidateEntry(entry,index=0){
      ...
      return {id:entry?.id||`candidate-${index+1}`,enabled:entry?.enabled!==false,mode:entry?.mode||'wall',rotations:Array.isArray(entry?.rotations)&&entry.rotations.length?entry.rotations:[0,90],relativeTo:entry?.relativeTo||'',relation:entry?.relation||'',side:['front','back','left','right'].includes(entry?.side)?entry.side:'front',crossAlign:['center','back','front','left','right'].includes(entry?.crossAlign)?entry.crossAlign:'center',distance:{min,max,step},facing:[...new Set(facing)],maxSamples:Math.min(48,Math.max(1,Math.round(Number(entry?.maxSamples)||12))),weight:Math.min(3,Math.max(0,Number.isFinite(Number(entry?.weight))?Number(entry.weight):1)),collisionClearance:Math.min(1,Math.max(0,Number.isFinite(Number(entry?.collisionClearance))?Number(entry.collisionClearance):.025)),allowFunctionalOverlap:!!entry?.allowFunctionalOverlap,compoundConstraint:entry?.compoundConstraint?clone(entry.compoundConstraint):undefined,referenceShapePolicy:{...(entry?.referenceShapePolicy||{}),lShape:policy},lShapeCrossAlign:policy.frontAlign,excludeForLShape:policy.enabled==null?null:!policy.enabled,avoidChaiseSide:policy.lateralSide==null?null:policy.lateralSide==='non-chaise'};
    }
    ...
    function normalizeRule(rule,index=0){
      ...
      if(Array.isArray(value.geometry.variants))value.geometry.variants=value.geometry.variants.filter(row=>row&&row.shape).map((row,index)=>({id:row.id||`${row.shape}-${index+1}`,label:row.label||row.id||row.shape,width:Math.max(.1,finite(row.width??row.w,value.geometry.width)),depth:Math.max(.1,finite(row.depth??row.d,value.geometry.depth)),shape:row.shape}));value.candidate=value.candidate||{mode:'wall',rotations:[0,90],relativeTo:'',relation:'',gap:0};value.candidate.rules=candidateRulesFromCandidate(value.candidate);value.candidate.ruleListVersion=2;value.candidate.maxCandidates=Math.min(72,Math.max(4,Math.round(Number(value.candidate.maxCandidates)||32)));Object.assign(value.candidate,clone(value.candidate.rules[0]));value.placement={requiredAnchor:'none',avoidWindow:false,allowCorner:false,...value.placement};value.placement.allowCorner=!!value.placement.allowCorner;value.service=value.service||{label:'日常使用区',side:'front',depth:.42,spanExtra:0,hard:false,allowBodyTypes:[]};
      ...
      if(value.program==='living'&&value.id==='arm'){
        value.candidate.rules=value.candidate.rules.filter(entry=>!(entry.relativeTo==='sofa'&&(entry.side==='left'||entry.side==='right'))&&entry.side!=='outward');
      ...
      if(value.program==='bedroom'&&value.id==='bench'){
        ...
        value.candidate.rules=[candidateRule('bench-bed-foot-near','relation',{relativeTo:'bed',relation:'bed-foot',side:'front',distance:{min:.10,max:.15,step:.05},facing:['toward'],maxSamples:4,allowFunctionalOverlap:true,weight:1.42})];value.candidate.maxCandidates=4;
        Object.assign(value.candidate,clone(value.candidate.rules[0]));
      }
    ...
    function valueFromForm(){
      syncCandidateRuleFromForm();const run=$('runEnabled').checked?{min:number('runMin',.8),max:number('runMax',2.4),step:number('runStep',.1),fill:[number('fillMin',.6),number('fillMax',.92)],maxByWalls:$('runMaxByWalls').checked,maxPerWall:Math.max(1,Math.round(number('runMaxPerWall',1))),wallCountCap:Math.max(1,Math.round(number('runWallCountCap',4))),areaPerCabinet:Math.max(2,number('runAreaPerCabinet',10))}:null,first=candidateRulesDraft[0]||normalizeCandidateEntry({},0);
      return {schemaVersion:1,program:$('ruleProgram').value,id:$('ruleId').value.trim(),label:$('ruleLabel').value.trim(),category:$('ruleCategory').value.trim(),role:$('ruleRole').value.trim(),color:$('ruleColor').value,
        geometry:{width:number('width',1),depth:number('depth',.4),shape:$('shape').value,variants:[...$('shapeVariants').querySelectorAll('input:checked')].map(input=>input.value==='box'?{id:'box',label:$('ruleLabel').value.trim()||'普通矩形',width:number('width',1),depth:number('depth',.4),shape:'box'}:{id:input.value,label:input.value==='l-left'?'左贵妃 L 形':'右贵妃 L 形',width:number('lVariantWidth',2.8),depth:number('lVariantDepth',1.65),shape:input.value})},quantity:{min:Math.round(number('minCount',0)),max:Math.round(number('maxCount',1))},preferences:{defaultCount:Math.round(number('defaultCount',0)),weight:number('preferenceWeight',1),priority:Math.round(number('placementPriority',100))},accessTarget:$('accessTarget').checked,
        candidate:{...clone(first),gap:first.distance.min,rules:clone(candidateRulesDraft),maxCandidates:Math.min(72,Math.max(4,Math.round(number('candidateTotalLimit',32)))),ruleListVersion:2},
        placement:{requiredAnchor:$('requiredAnchor').value,avoidWindow:$('avoidWindow').checked,allowCorner:$('allowCorner').checked},
        service:{label:$('serviceLabel').value.trim(),side:$('serviceSide').value,depth:number('serviceDepth',.42),spanExtra:number('spanExtra',0),alignStart:$('serviceAlignStart').checked,adaptiveFootZone:$('adaptiveFootZone').checked,adaptiveGapThreshold:Number(original?.service?.adaptiveGapThreshold)||.42,hard:$('serviceHard').checked,allowBodyTypes:[...$('allowBodyTypes').querySelectorAll('input:checked')].map(input=>input.value)},surface:clone(original?.surface||null),run};
    }
    function setForm(rule){
      ...
      populateFurnitureEnums(rule.program,rule.id,rule.candidate?.relativeTo||'',rule.service?.allowBodyTypes||[]);candidateRulesDraft=candidateRulesFromCandidate(rule.candidate);selectedCandidateRuleIndex=0;
      $('width').value=rule.geometry.width;$('depth').value=rule.geometry.depth;$('shape').value=rule.geometry.shape||'box';const variants=rule.geometry.variants||[],variantShapes=new Set(variants.map(row=>row.shape));$('shapeVariants').querySelectorAll('input').forEach(input=>input.checked=variantShapes.has(input.value));const lVariant=variants.find(row=>row.shape?.startsWith('l-'));$('lVariantWidth').value=lVariant?.width??Math.max(2.8,rule.geometry.width+.5);$('lVariantDepth').value=lVariant?.depth??Math.max(1.65,rule.geometry.depth+.65);$('minCount').value=rule.quantity.min;$('maxCount').value=rule.quantity.max;$('defaultCount').value=rule.preferences?.defaultCount??rule.quantity.min;$('preferenceWeight').value=rule.preferences?.weight??1;$('placementPriority').value=rule.preferences?.priority??100;$('accessTarget').checked=!!rule.accessTarget;
    ...
    function renderLive(){const rule=valueFromForm(),errors=validate(rule);$('jsonPreview').textContent=JSON.stringify(rule,null,2);renderPreview(rule);const seatFacing=rule.candidate.mode==='relation'&&/desk|table|vanity/i.test(rule.candidate.relativeTo||'');$('orientationHint').textContent=seatFacing?`方向说明：${rule.label||rule.id}的正面朝向“${rule.candidate.relativeTo}”；“${rule.service.label||'使用区'}”设为${rule.service.side==='back'?'背面时位于远离桌面的一侧，用于拉椅和起身。':'正面时位于家具与桌面之间。'}`:'方向说明：使用区方向以家具自身“正面”为基准，而不是以画布上下方向为基准。';status.className=`status ${errors.length?'error':'ok'}`;status.textContent=errors.length?errors.join('；'):'规则结构有效；停止输入约 0.5 秒后自动保存并应用。'}
