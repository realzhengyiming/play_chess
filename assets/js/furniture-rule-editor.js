  (async()=>{
    'use strict';
    // 服务器配置是只读基线；每个浏览器维护自己的实验副本。
    const ENABLE_LOCAL_CONFIG_PERSISTENCE=true;
    ...
    function globalConfigPayload(profile=currentProfile()){
      return {schemaVersion:9,baselineVersion:BASELINE_VERSION,profileName:'浏览器用户配置',updatedAt:new Date().toISOString(),designQualityRules:clone(globalDesignQualityRules),layoutConstraints:clone(globalLayoutConstraints),roomTypes:clone(profile?.roomTypes||DEFAULT_ROOM_TYPES),furnitureLibrary:catalog.map(compactRule),roomAssignments:clone(profile?.roomAssignments||{}),roomSettings:clone(profile?.roomSettings||{})};
    }
    function applyGlobalConfigPayload(parsed,{name='我的浏览器配置'}={}){
      ...
      const profile=normalizeProfile({...parsed,id:'user-profile',name,catalog:normalizeCatalog(rows),roomTypes:Array.isArray(parsed.roomTypes)?parsed.roomTypes:DEFAULT_ROOM_TYPES,roomAssignments:parsed.roomAssignments,roomSettings:parsed.roomSettings,updatedAt:parsed.updatedAt||new Date().toISOString()});
    ...
    async function initializeGlobalConfig(){
      try{
        ...
        let source=payload.config,restored=false;
        try{const stored=localStorage.getItem(USER_GLOBAL_CONFIG_KEY);if(stored){const parsed=JSON.parse(stored);globalThis.RoomChessConfigContract.assertGlobalConfig(parsed);source=parsed}else restored=true}catch(error){localStorage.removeItem(USER_GLOBAL_CONFIG_KEY);restored=true;}
        applyGlobalConfigPayload(source);serverReady=true;if(restored)localStorage.setItem(USER_GLOBAL_CONFIG_KEY,JSON.stringify(globalConfigPayload()));localStorage.setItem(ACTIVE_CONFIG_SOURCE_KEY,'user');status.className='status ok';status.textContent=restored?'已从服务器全局配置创建当前浏览器的用户副本。':'已读取当前浏览器的用户配置；修改不会覆盖服务器。';
    ...
    function saveGlobalConstraints(){
      const output=$('globalConstraintsStatus');try{const parsed=JSON.parse($('globalConstraintsJson').value),errors=validateConstraintDocument(parsed);if(errors.length)throw new Error(errors.join('；'));globalDesignQualityRules=clone(parsed.designQualityRules);globalLayoutConstraints=clone(parsed.layoutConstraints);persist();renderGlobalConstraints();output.className='status ok';output.textContent='结构有效，已保存到当前浏览器的用户配置。';}catch(error){output.className='status error';output.textContent=`未保存：${error.message}`;}
    }
    ...
    function renderDistanceGuide(){
      const min=Math.max(0,number('relationGapMin',0)),max=Math.max(min,number('relationGapMax',min)),step=Math.max(.05,number('relationGapStep',.2)),scale=Math.max(1,Math.ceil(max*10)/10),count=Math.max(1,Math.floor((max-min+.0001)/step)+1),guide=$('candidateDistanceGuide');
      guide.style.setProperty('--range-start',`${Math.min(100,min/scale*100)}%`);guide.style.setProperty('--range-end',`${Math.min(100,max/scale*100)}%`);$('candidateDistanceLabel').textContent=`${min.toFixed(2)}–${max.toFixed(2)} m · 每次增加 ${step.toFixed(2)} m · 约 ${count} 个距离档位（标尺上限 ${scale.toFixed(1)} m）`;
    }
    function updateCandidateEditorUI(){
      const relationMode=$('candidateMode').value==='relation',relativeTo=$('relativeTo').value||'',lShapeRelation=relationMode&&/sofa/i.test(relativeTo);
      document.querySelectorAll('.relation-only').forEach(node=>node.hidden=!relationMode);document.querySelectorAll('.non-relation-only').forEach(node=>node.hidden=relationMode);document.querySelectorAll('.l-shape-only').forEach(node=>node.hidden=!lShapeRelation);renderDistanceGuide();
    }
    ...
    function renderCandidateEstimate(){
      const entry=candidateRulesDraft[selectedCandidateRuleIndex];if(!entry)return;const totalLimit=Math.max(1,Number($('candidateTotalLimit').value)||32);$('candidateEstimate').textContent=`本条规则最多保留 ${Math.max(1,Number(entry.maxSamples)||12)} 个候选，家具总计最多 ${totalLimit} 个。性能与碰撞细节可在“高级设置”中调整。`;
    }
    ...
    function toggleRun(){const enabled=$('runEnabled').checked;$('runFields').style.opacity=enabled?'1':'.38';$('runFields').querySelectorAll('input').forEach(input=>input.disabled=!enabled);if(enabled)$('runAdvancedSettings').open=true}
    ...
    function save({automatic=false,silent=false}={}){
      ...
      selectedId=rule.id;persist();original=clone(rule);renderList();renderLive();const time=new Date().toLocaleTimeString('zh-CN',{hour12:false});$('autoSaveKicker').textContent=`已自动保存 ${time}`;if(!silent){status.className='status ok';status.textContent=automatic?'已保存到当前浏览器；主页切换到“用户配置”即可使用。':'已保存到浏览器用户配置。'}return true;
    }
    ...
    fields.forEach(id=>$(id).addEventListener('input',()=>{if(id==='runEnabled')toggleRun();if(['candidateMode','relativeTo','relationGapMin','relationGapMax','relationGapStep'].includes(id))updateCandidateEditorUI();scheduleAutoSave()}));
    ...
    $('restoreDefaultsBtn').addEventListener('click',async()=>{if(!confirm('用服务器当前全局配置重建这个浏览器的用户配置？你的本地实验改动会被覆盖。'))return;try{const response=await fetch(GLOBAL_CONFIG_API,{cache:'no-store'}),payload=await response.json();if(!response.ok)throw new Error(payload.detail||`HTTP ${response.status}`);applyGlobalConfigPayload(payload.config);pendingDefaultKeys.clear();activeRoomTypeId='bedroom';selectedId=catalog[0]?.id||null;persist();renderList();setForm(active());renderGlobalConstraints();status.className='status ok';status.textContent='用户配置已恢复为服务器当前全局配置，并保存到这个浏览器。';}catch(error){status.className='status error';status.textContent=`恢复失败：${error.message}`;}});
