  (async()=>{
    ...
    function normalizeProfile(row){
      ...
      if(row.bedroomMediaPriorityV4!==true){
      ...
      if(row.livingDiningPriorityV3!==true){
      ...
      if(row.livingDiningPriorityV3!==true){const current=roomAssignments.living||[],rank=new Map(['sofa','tv','coffee','diningTable','diningChair','arm','side','floorLamp','sideboard','bookcase','display','console','ottoman','plant','infillCabinet','livingRug'].map((id,index)=>[id,index]));roomAssignments.living=[...current].sort((a,b)=>(rank.get(a)??999)-(rank.get(b)??999)||current.indexOf(a)-current.indexOf(b));roomAssignments.living.forEach((id,index)=>{roomSettings.living||={};roomSettings.living[id]={...(roomSettings.living[id]||{}),priority:(index+1)*10}})}
      if(row.bedroomMediaPriorityV4!==true){const current=roomAssignments.bedroom||[],rank=new Map(['bed','night','wardrobe','desk','chair','tvbench','bench','bedroomLoveseat','bedroomTeaTable','chest','shelf','bedroomDisplay','lounge','vanity','vanityStool','bedroomInfillCabinet'].map((id,index)=>[id,index]));roomAssignments.bedroom=[...current].sort((a,b)=>(rank.get(a)??999)-(rank.get(b)??999)||current.indexOf(a)-current.indexOf(b));roomAssignments.bedroom.forEach((id,index)=>{roomSettings.bedroom||={};roomSettings.bedroom[id]={...(roomSettings.bedroom[id]||{}),priority:(index+1)*10}})}
      ...
      for(const [key,rule] of Object.entries(defaultsByRule))defaultsByRule[key]=normalizeRule(rule);for(const rule of catalog)defaultsByRule[ruleKey(rule)]||=clone(rule);return {...row,baselineVersion:BASELINE_VERSION,richBedroomDefaultsV1:true,richBedroomDefaultsV2:true,wallCabinetFirstV1:true,bedroomCorePriorityV1:true,livingCorePriorityV1:true,livingGroupDependencyV2:true,bedroomGroupDependencyV3:true,bedroomMediaPriorityV4:true,livingDiningPriorityV3:true,catalog,defaultsByRule,roomTypes,roomAssignments,roomSettings};
    }
