  (async()=>{
    ...
    const DEFAULT_LAYOUT_CONSTRAINTS={
      ...
      designGrammar:{
        living:{groups:{lounge:{anchor:'sofa',members:['sofa','tv','coffee','arm','side','ottoman','floorLamp'],compact:[.12,.30]},dining:{anchor:'diningTable',members:['diningTable','diningChair','sideboard'],compact:[.10,.25]}},zones:{wide:{lounge:[.34,.49],dining:[.73,.51]},deep:{lounge:[.50,.34],dining:[.51,.73]}},pairs:{sofaCoffee:{gap:[.35,.40],compact:[.30,.40],hardMin:.28},sofaTv:{gap:[1.55,3.20]},conversation:{radius:[.82,1.62]},diningSideboard:{distance:[.65,3.0]}}},
    ...
    function geometryFromForm(){
      const width=number('width',1),depth=number('depth',.4),shape=$('shape').value,previous=clone(original?.geometry||{}),previousVariants=Array.isArray(previous.variants)?previous.variants:[],selected=[...$('shapeVariants').querySelectorAll('input:checked')].map(input=>input.value),variants=[];
      for(const variantShape of selected){
        if(variantShape==='box'){
          const boxes=previousVariants.filter(row=>row.shape==='box');
          if(boxes.length>1){
            for(const row of boxes){const isBase=Math.abs(Number(row.width??row.w)-Number(previous.width))<1e-6&&Math.abs(Number(row.depth??row.d)-Number(previous.depth))<1e-6;variants.push(isBase?{...row,width,depth}:{...row})}
          }else variants.push({...boxes[0],id:boxes[0]?.id||'box',label:boxes[0]?.label||$('ruleLabel').value.trim()||'普通矩形',width,depth,shape:'box'});
        }else{
          const saved=previousVariants.find(row=>row.shape===variantShape)||{};
          variants.push({...saved,id:saved.id||variantShape,label:saved.label||(variantShape==='l-left'?'左贵妃 L 形':'右贵妃 L 形'),width:number('lVariantWidth',2.8),depth:number('lVariantDepth',1.65),shape:variantShape});
        }
      }
      return {...previous,width,depth,shape,variants,searchVariants:previous.searchVariants??variants.length>1};
    }
    ...
    function valueFromForm(){
      ...
      return {...clone(original||{}),schemaVersion:1,program:$('ruleProgram').value,id:$('ruleId').value.trim(),label:$('ruleLabel').value.trim(),category:$('ruleCategory').value.trim(),role:$('ruleRole').value.trim(),color:$('ruleColor').value,
        ...
        candidate:{...clone(original?.candidate||{}),...clone(first),rules:clone(candidateRulesDraft),maxCandidates:Math.min(72,Math.max(4,Math.round(number('candidateTotalLimit',32)))),ruleListVersion:2},
