...
项目使用 [uv](https://docs.astral.sh/uv/) 管理 Python 版本、虚拟环境和依赖。服务器需预装 `uv` 和 Node.js（全局配置接口会调用 Node.js 校验规则）。首次拉取代码后，在项目目录运行：
...
`uv sync --frozen` 严格按照仓库内的 `uv.lock` 创建 `.venv`，后续启动只需运行第二条命令。默认监听 `0.0.0.0:8765`；本机浏览器仍使用 `http://127.0.0.1:8765/` 访问。

开发时自动重载：

```powershell
uv run --frozen uvicorn floorplan_api:app --host 127.0.0.1 --port 8765 --reload
```
...
```bash
uv sync --frozen
ROOM_CHESS_HOST=0.0.0.0 ROOM_CHESS_PORT=8765 uv run --frozen room-chess-server
```

也可以直接使用 Uvicorn 参数：

```bash
uv run --frozen uvicorn floorplan_api:app --host 0.0.0.0 --port 8765
```
...
uv run --frozen python -m py_compile floorplan_api.py
```

新增户型图片时，可将文件按 `case_编号_面积.jpg/png` 命名后放入 `样例图片数据`，启动本地服务并运行：

```powershell
uv run --frozen python scripts/batch_recognize_samples.py
```
